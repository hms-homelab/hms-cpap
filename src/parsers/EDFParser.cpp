...
bool EDFParser::parsePLDFile(const std::string& filepath, CPAPSession& session) {
    std::cout << "Parser: Parsing PLD " << filepath << std::endl;

    EDFFile edf;
    if (!edf.open(filepath)) {
        std::cout << "Parser: PLD file could not be opened (may be incomplete)" << std::endl;
        return true;  // PLD is optional, don't fail session
    }
    ...
    if (n_samples == 0) {
        std::cout << "Parser: PLD file has no data samples" << std::endl;
        return true;
    }
    ...
    std::cout << "Parser: PLD parsed — " << n_minutes << " minutes of machine data"
              << " (" << n_samples << " samples)" << std::endl;
    if (tgt_vent_idx >= 0) {
        std::cout << "Parser: PLD includes TgtVent (ASV device)" << std::endl;
    }
...
std::unique_ptr<CPAPSession> EDFParser::parseSession(
    ...
    if (!pld_files.empty()) {
        std::cout << "Parser: Processing " << pld_files.size() << " PLD checkpoint(s)..." << std::endl;
        for (size_t i = 0; i < pld_files.size(); ++i) {
            std::cout << "  PLD " << (i+1) << "/" << pld_files.size() << ": "
                      << std::filesystem::path(pld_files[i]).filename() << std::endl;

            if (!parsePLDFile(pld_files[i], *session)) {
                std::cerr << "  Failed to parse PLD, skipping" << std::endl;
                continue;
            }
        }
    }
...
// ============================================================================
//  STR.edf parser — daily therapy summaries (81 signals, 1 record/day)
// ============================================================================
...
std::vector<STRDailyRecord> EDFParser::parseSTRFile(
    ...
    std::cout << "STR: Opened " << filepath << " — " << edf.actual_records
              << " days, " << edf.num_signals << " signals" << std::endl;
    ...
    // ResMed stores local time in the EDF header
    ...
    std::cout << "STR: Parsed " << therapy_days << " therapy days from "
              << edf.actual_records << " total records" << std::endl;
