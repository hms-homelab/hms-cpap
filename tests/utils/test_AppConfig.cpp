...
TEST_F(AppConfigTest, EnvVarFallbackForEmptyDbFields) {
    ...
    saveAndSetEnv("DB_HOST", "127.0.0.1");
...
TEST_F(AppConfigTest, EnvVarFallbackForMqtt) {
    saveAndSetEnv("MQTT_BROKER", "127.0.0.1");
    ...
    EXPECT_EQ(config.mqtt.broker, "127.0.0.1");
...
TEST_F(AppConfigTest, SaveAndReloadPreservesValues) {
    ...
    original.mqtt.broker = "127.0.0.1";
    ...
    EXPECT_EQ(loaded.database.host, "127.0.0.1");
    EXPECT_EQ(loaded.database.port, 5432);
    EXPECT_TRUE(loaded.mqtt.enabled);
    EXPECT_EQ(loaded.mqtt.broker, "127.0.0.1");
}
